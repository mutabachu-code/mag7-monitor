...
# Define the logical core as a System Instruction (Permanent Logic)
SYSTEM_LOGIC = """
You are an expert forex analyst specializing in Volume Profile and institutional order flow.
Analyze the setup and provide a structured trade decision in JSON.
...
TRADING RULES:
- MEAN REVERSION: Need volume exhaustion at VA edges + trend alignment.
- BREAKOUT: Need price acceptance outside VA + low ATR entry.
- RISK/REWARD: Minimum 1.5:1. Account for provided spread.
"""
...
def analyse_pair(vp: VolumeProfile, lot_size: float = 0.02,
    ...
    # Focus prompt on dynamic dashboard data
    prompt = f"""
...
TASK: Check latest news for {base_ccy} and {quote_ccy} for April 28, 2026. 
Identify Central Bank stance (Fed/ECB/BOJ) and risk sentiment.
Return JSON ONLY.
"""
