...
for idx, pair in enumerate(PAIRS):
    ...
    with cols[idx % 3]:
        ...
        # Only skip truly neutral signals — all Breakout and Mean Reversion go to Claude
        ...
        if should_analyse:
            with st.expander("🤖 Claude AI Analysis", expanded=True):
                ...
                if ai is None and session_ok:
                    # Claude was called but returned None — explain why
                    st.caption("🤖 Claude analysed this signal and found conditions incomplete.")
                    st.caption("Waiting for: volume exhaustion confirmation OR price acceptance outside VA for 30+ min.")
                elif ai is not None:
                    # CB sentiment badge
